...
class DataMgr:
    ...
    def __init__(
        self,
        static_dir: str = "gce/main/Instrument Static",
        dat_path: str = "InstrumentStatic.dat",
        db_path: str = "rms_limits.db",
        ini_path: str = "gce/main/config/Datamgr.ini",
        auto_load: bool = True,
    ):
        """
        Initialize DataMgr.
    ...
    def load_session_config(self, ini_path: Optional[str] = None) -> int:
        ...
        """
        target_path = ini_path or self.ini_path or "gce/main/config/Datamgr.ini"
        path = Path(target_path)

        if not path.exists():
            if Path("gce/main/config/Datamgr.ini").exists():
                path = Path("gce/main/config/Datamgr.ini")
