...
MAIN_ROOT = Path(PROJECT_ROOT) / "gce" / "main"
...
# GCE imports
from gce.main.cache.order_cache import OrderCache, Order, OrderStatus
...
from gce.main.reconciler import PositionReconciler
from gce.main.engine import GCE

from gce.main.gui.log_parser import LogParser
    ...
    with _lock:
        ...
        try:
            _state["datamgr"] = DataMgr(
                static_dir=str(MAIN_ROOT / "Instrument Static"),
                dat_path=str(MAIN_ROOT / "cache" / "InstrumentStatic.dat"),
            )
            _state["datamgr"].load_session_config(
                str(MAIN_ROOT / "config" / "Datamgr.ini")
            )
        ...
        try:
            ...
            if _state["instruments"].count() == 0 and (MAIN_ROOT / "Instrument Static" / "HK-ListOfSecurities.csv").exists():
                _state["instruments"].load_from_csv(str(MAIN_ROOT / "Instrument Static" / "HK-ListOfSecurities.csv"))
        except Exception:
        ...
        try:
            ...
            _state["pxfeeder"] = PXFeeder(
                dat_path=str(MAIN_ROOT / "cache" / "PriceCache.dat"),
        ...
        try:
            ...
            _state["prices"] = PriceCache(dat_path=str(MAIN_ROOT / "cache" / "PriceCache.dat"))

        # OrderCache
        try:
            _state["orders"] = OrderCache(
                dat_path=str(MAIN_ROOT / "cache" / "OrderCache.dat"),
                instrument_cache=_state["instruments"],
            )
        except Exception:
            _state["orders"] = OrderCache(dat_path=str(MAIN_ROOT / "cache" / "OrderCache.dat"))

        # PositionCache
        try:
            _state["positions"] = PositionCache(
                dat_path=str(MAIN_ROOT / "cache" / "PositionsCache.dat"),
            )
        except Exception:
            _state["positions"] = PositionCache(dat_path=str(MAIN_ROOT / "cache" / "PositionsCache.dat"))
...
def api_update_config():
    ...
    try:
        ...
        config_file = MAIN_ROOT / "config" / filename
...
def api_limits_export():
    ...
    from gce.main.datamgr import ALL_DB_COLUMNS
    rows = dm.get_all_limits_from_db()
...
def api_orders_place():
    ...
    try:
        ...
        if orders_cache:
            try:
                orders_cache.save_to_dat(str(MAIN_ROOT / "cache" / "OrderCache.dat"))
            except Exception as e:
    ...
    try:
        ...
        if px:
            ...
            if p_data and float(p_data.get("last", 0.0) or 0.0) > 0:
                if pc and hasattr(pc, "update_price_in_memory"):
                    ...
                    pc.save_to_dat(str(MAIN_ROOT / "cache" / "PriceCache.dat"))
        ...
        if details and float(details.get("last", 0.0) or 0.0) > 0:
            if px and hasattr(px, "update_price_in_memory"):
                ...
                px.save_to_dat(str(MAIN_ROOT / "cache" / "PriceCache.dat"))
            if pc and hasattr(pc, "update_price_in_memory"):
...
def api_sessions_update():
    ...
    if dm:
        try:
            dm.load_session_config(str(MAIN_ROOT / "config" / "Datamgr.ini"))
...
def api_admin_purge_oms():
    ...
    try:
        ...
        for csv_name in ("gce/main/cache/OrderCache.csv", "OrderCache.csv"):
        ...
        for dat_name in ("gce/main/cache/OrderCache.dat", "OrderCache.dat"):
...
def api_admin_purge_positions():
    ...
    try:
        ...
        for csv_name in ("gce/main/cache/PositionsCache.csv", "PositionsCache.csv"):
            p = Path(PROJECT_ROOT) / csv_name
            if p.exists():
                try:
                    p.unlink()
        ...
        # Reset .dat cache file
        for dat_name in ("gce/main/cache/PositionsCache.dat", "PositionsCache.dat"):
            p = Path(PROJECT_ROOT) / dat_name
            if p.exists():
                try:
                    import pickle
...
def api_admin_purge_prices():
    ...
    try:
        ...
        for csv_name in ("gce/main/cache/PriceCache.csv", "PriceCache.csv"):
            p = Path(PROJECT_ROOT) / csv_name
            if p.exists():
                try:
                    p.unlink()
        ...
        # Reset .dat cache file
        for dat_name in ("gce/main/cache/PriceCache.dat", "PriceCache.dat"):
            p = Path(PROJECT_ROOT) / dat_name
            if p.exists():
                try:
                    import pickle
...
def api_admin_purge_instruments():
    ...
    try:
        ...
        for dat_name in ("gce/main/cache/InstrumentStatic.dat", "InstrumentStatic.dat"):
            p = Path(PROJECT_ROOT) / dat_name
            if p.exists():
                try:
                    p.unlink()
