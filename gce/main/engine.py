...
from gce.main.controls.base_control import BaseControl, ControlExecution, ControlResult
from gce.main.cache.order_cache import Order, OrderStatus
from gce.main.cache import InstrumentCache, PriceCache, OrderCache, PositionCache
...
class GCE:
    ...
    def __init__(self, instrument_csv: str = "HK-ListOfSecurities.csv",
                 price_csv: str = "gce/main/cache/PriceCache.csv",
                 order_csv: str = "gce/main/cache/OrderCache.csv",
                 position_csv: str = "gce/main/cache/PositionsCache.csv",
                 log_dir: str = "logs",
                 max_workers: int = 8):
        """
        ...
        # Load caches
        try:
            self.datamgr = DataMgr(static_dir="gce/main/Instrument Static", dat_path="gce/main/cache/InstrumentStatic.dat")
        ...
        try:
            ...
            self.pxfeeder = PXFeeder(dat_path="gce/main/cache/PriceCache.dat", symbols=default_symbols, fetch_on_start=False, auto_start_bg=True)
            self.prices = PriceCache(dat_path="gce/main/cache/PriceCache.dat", csv_path=price_csv, fetch_yfinance=False, symbols=default_symbols, auto_save=True)
            ...
            self.pxfeeder = PXFeeder(dat_path="gce/main/cache/PriceCache.dat", fetch_on_start=False, auto_start_bg=False)
            self.prices = PriceCache(dat_path="gce/main/cache/PriceCache.dat")

        try:
            self.orders = OrderCache(dat_path="gce/main/cache/OrderCache.dat", instrument_cache=self.instruments)
        ...
        try:
            self.positions = PositionCache(dat_path="gce/main/cache/PositionsCache.dat")
    ...
    def validate_order(self, order: Order, is_new: bool = True, parallel: bool = True) -> Tuple[bool, List[str]]:
        ...
        """
        from gce.main.rule_engine import RuleEngine
        ...
        if ric and hasattr(self, 'pxfeeder') and self.pxfeeder:
            ...
            if px and hasattr(self, 'prices') and self.prices:
                ...
                target_dat = getattr(self.prices, 'dat_path', None) or "gce/main/cache/PriceCache.dat"
        ...
        if order_passed:
            ...
            if hasattr(self, 'positions') and self.positions:
                ...
                try:
                    self.positions.save_to_dat("gce/main/cache/PositionsCache.dat")
                except Exception as e:
    ...
    def save_state(self, order_dat: str = "gce/main/cache/OrderCache.dat",
                   position_dat: str = "gce/main/cache/PositionsCache.dat") -> None:
    ...
    def get_risk_report(self):
        ...
        from gce.main.analytics import RiskAnalytics
